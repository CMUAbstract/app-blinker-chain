...
void init()
{
    ...
    GPIO(PORT_LED2, DIR) |= BIT(PIN_LED2);
#if defined(PORT_LED3)
    GPIO(PORT_LED3, DIR) |= BIT(PIN_LED3);
#endif
...
#if defined(PORT_LED3) // when available, this LED indicates power-on
    GPIO(PORT_LED3, OUT) |= BIT(PIN_LED3);
#endif
}
